...
# COLORS = [C_RED, C_BLUE, C_GREEN, C_EXPYRIMENT_ORANGE]
...
for block_id in range(1, N_BLOCKS + 1):
    for trial_id in range(1, N_TRIALS_IN_BLOCK + 1):
        trial_type = rand_element(TRIAL_TYPES)
        word = rand_element(COLORS)
        color = word if trial_type == 'match' else rand_element([c for c in COLORS if c != word])
